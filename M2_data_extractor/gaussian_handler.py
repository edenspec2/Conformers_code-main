...
os.chdir(r'C:\Users\edens\Documents\GitHub\Conformers_code-main\M2_data_extractor\gauss_log')
...
class Names(Enum):
    ...
    DF_LIST=['standard_orientation_df', 'dipole_df', 'pol_df', 'atype_df','charge_df', 'bonds_df', 'info_df']
...
def process_gaussian_charge(log_file_lines):
    ...
    selected_lines = np.array(extract_lines_from_text(log_file_lines[charges_start.start():charges_end.start()],
                                             re_expression=ReExpressions.FLOATS_ONLY.value))
    selected_lines=selected_lines[1::6]
    return pd.DataFrame(selected_lines, columns=['charge'], dtype=float)
...
def gauss_file_handler(gauss_filename, export=False):
    with open(os.path.abspath(gauss_filename)) as f:
        ...
        #close file
        f.close()
    charge_df = process_gaussian_charge(log_file_lines)
    ...
    # bonds_df=process_gaussian_bonds(gauss_data[4])  ## should be this with no NBO ???
    # frequency_string=process_gaussian_frequency_string(gauss_data[3])
    bonds_df=process_gaussian_bonds(gauss_data[8])
    frequency_string=process_gaussian_frequency_string(gauss_data[7])
...
if __name__ == '__main__':
    ...
    parameters=gauss_file_handler(r'Et.log',export=False)
