...
def calc_coordinates_transformation(coordinates_array: npt.ArrayLike, base_atoms_indices: npt.ArrayLike, round_digits:int=4) -> npt.ArrayLike:#origin_atom, y_direction_atom, xy_plane_atom
    """
...
def calc_npa_charges(coordinates_array: npt.ArrayLike,charge_array: npt.ArrayLike, base_atoms_indices: npt.ArrayLike, sub_atoms=None):##added option for subunits
    """
...
#TODO: Add option for sub_atoms!
    dipole_xyz = np.vstack([(row[0] * row[1])for row in
                            list(zip(transformed_coordinates, charge_array))])
    # dipole_xyz = np.sum(np.multiply(transformed_coordinates[atom_mask, :], charge_array[atom_mask, np.newaxis]),
    #                     axis=0)
    dipole_vector=np.sum(dipole_xyz,axis=0)
    array_dipole=np.hstack([dipole_vector,np.linalg.norm(dipole_vector)])
    dipole_df=pd.DataFrame(array_dipole,index=help_functions.XYZConstants.DIPOLE_COLUMNS.value)
    return dipole_df
...
def calc_dipole_gaussian(coordinates_array, gauss_dipole_array, base_atoms_indices):
    ...
    return pd.DataFrame(gauss_dipole_array,columns=help_functions.XYZConstants.DIPOLE_COLUMNS.value).T
...
def vibrations_dict_to_list(vibration_dict,vibration_atom_nums):
    ...
    vibration_array_pairs=list(zip(vibration_array_list[::2], vibration_array_list[1::2])) #[::2] means every second element
...
def calc_vibration_dot_product(vibration_array_pairs,coordinates_vector):
    vibration_dot_product_itter=zip(coordinates_vector, vibration_array_pairs)
    vibration_dot_product=[abs(np.dot(pair[0], coord_vector))+abs(np.dot(pair[1], coord_vector)) for coord_vector, pair in vibration_dot_product_itter]
    # vibration_dot_product=[(abs(np.dot(pair[0],coordinates_vector[i]))+abs(np.dot(pair[1],coordinates_vector[i]))) for
    #                        i,pair in list(enumerate(vibration_array_pairs))]
    return vibration_dot_product

def flatten_list(nested_list_arg):
    flat_list=[item for sublist in nested_list_arg for item in sublist]
    return flat_list

def calc_vibration_dot_product_from_pairs(coordinates_array,vibration_dict,atom_pairs):
    atom_pairs=adjust_indices(atom_pairs)
    vibration_atom_nums=flatten_list(atom_pairs)
    ...
    coordinates_vector=indices_to_coordinates_vector(coordinates_array,atom_pairs)
    vibration_dot_product=calc_vibration_dot_product(vibration_array_pairs,coordinates_vector)
    return vibration_dot_product

# def filter_info_df_frequency(info_df, threshhold):
#     info_df=info_df.T
#     outer_finger=(info_df.iloc['Frequency'].astype(float)>threshhold)
#     filtered_info_df=info_df[outer_finger]
...
def calc_max_frequency_gen_vibration(vibration_dot_product,info_df,threshhold=500): ## fix so 0 is IR and 1 is frequency
   ...
   2      29.0304  0.0019
    ...
    """
    # Make a lot better!! work df-df and not action-action

    # vibration_dot_product=vibration_dot_product[(filtered_info_df.index.tolist())[0]:list(filtered_info_df.index.tolist())[-1]+1]

    df_list=[pd.DataFrame(np.vstack([vibration,(info_df)['Frequency']]),index=help_functions.XYZConstants.VIBRATION_INDEX.value).T
             for vibration in vibration_dot_product]
    filtered_df_list=[df[(df[help_functions.XYZConstants.VIBRATION_INDEX.value[1]]>threshhold)] for df in df_list]
    index_max=[df[help_functions.XYZConstants.VIBRATION_INDEX.value[0]].idxmax() for  df in df_list]
    max_frequency_vibration=[pd.DataFrame(list(df.iloc[index_max[i]]),index=help_functions.XYZConstants.VIBRATION_INDEX.value) for i,df in enumerate(df_list)]
...
def get_data_for_ring_vibration(info_df,vibration_array_list,coordinates_vector)->pd.DataFrame:
    ...
    data_df=pd.DataFrame(np.vstack([product,(info_df)['Frequency'],sin_angle]),index=help_functions.XYZConstants.RING_VIBRATION_DATA_INDEX.value)
    return data_df.T


def get_filter_ring_vibration_df(data_df,prods_threshhold=0.1, frequency_min_threshhold=500, frequency_max_threshhold=700): ##return filtered_df and check
    # help_functions.XYZConstants.RING_VIBRATION_DATA_INDEX.value=['product','frequency','sin_angle']
    filter_prods_null=(data_df[help_functions.XYZConstants.RING_VIBRATION_DATA_INDEX.value[0]]!=0)
    filter_prods_value=(abs(data_df[help_functions.XYZConstants.RING_VIBRATION_DATA_INDEX.value[0]])>prods_threshhold)
    filter_min_frequency=(data_df[help_functions.XYZConstants.RING_VIBRATION_DATA_INDEX.value[1]]>frequency_min_threshhold) # wierd part - check about dot product
    filtered_df=data_df[filter_prods_null & filter_prods_value][filter_min_frequency].reset_index() ##randon threshhold to work, no >1500
    if (filtered_df.shape[0]==0):
        filter_max_frequency = (data_df[help_functions.XYZConstants.RING_VIBRATION_DATA_INDEX.value[1]]<frequency_max_threshhold)
...
def get_filtered_ring_df(info_df,coordinates_array,vibration_dict,ring_atom_indices):
    ring_atom_indices=adjust_indices(ring_atom_indices)
    coordinates_vector=indices_to_coordinates_vector(coordinates_array,ring_atom_indices)[0]
...
def calc_min_max_ring_vibration(filtered_df)->pd.DataFrame: ## last ring_vibration
    max_vibration_frequency=filtered_df.iloc[filtered_df[help_functions.XYZConstants.RING_VIBRATION_DATA_INDEX.value[2]].idxmax()][1] #[1] is product
    asin_max=math.asin(filtered_df[help_functions.XYZConstants.RING_VIBRATION_DATA_INDEX.value[2]].max())*(180/np.pi) 
    min_vibration_frequency=filtered_df.iloc[filtered_df[help_functions.XYZConstants.RING_VIBRATION_DATA_INDEX.value[2]].idxmin()][1]
    asin_min=math.asin(filtered_df[help_functions.XYZConstants.RING_VIBRATION_DATA_INDEX.value[2]].min())*(180/np.pi)
    df=pd.DataFrame((max_vibration_frequency,asin_max,min_vibration_frequency,asin_min),
                    index=help_functions.XYZConstants.RING_VIBRATION_INDEX.value)
    return df
...
def remove_nof_bonds(bonded_atoms_df,bonds_array,coordinates_df):
    ...
    for idx, row in enumerate(bonded_atoms_array):
        if (row[0]=='H') & (row[1] in help_functions.XYZConstants.NOF_ATOMS.value):
            nof_indices.append(idx)
        elif (row[1]=='H') & (row[0] in help_functions.XYZConstants.NOF_ATOMS.value):
            nof_indices.append(idx)
    ...
    for index in nof_indices:
        ...
        coordinates_array=coordinates_df[['x','y','z']].astype(float).to_numpy()
...
def get_extended_df_for_sterimol(coordinates_df,bond_type,radii='bondi'):
    """
...
def calc_sterimol(bonded_atoms_df,extended_df):
    ...
    sterimol_df=pd.DataFrame([B1,B5,L,loc_B5,loc_B1],index=help_functions.XYZConstants.STERIMOL_INDEX.value) ## loc_B1 produces different values
    return sterimol_df
...
class Molecule():
    ...
    def get_sterimol(self,base_atoms,radii='bondi'):
        ...
        longest_path=get_molecule_connections(self.bonds_df,bonds_direction[0],bonds_direction[1])
        extended_df=get_extended_df_for_sterimol(transformed_coordinates,self.atype_df,radii)
        bonded_atoms_df=get_specific_bonded_atoms_df(self.bonds_df,longest_path,self.xyz_df)
    ...
    def get_coordination_transformation_df(self,base_atoms_indices):
        ...
        new_coordinates_df=pd.DataFrame(new_coordinates_array,columns=help_functions.XYZConstants.XYZ_COLUMNS.value)
    ...
    def get_gen_vibration(self,atom_pairs):
        ...
        if check_pair_in_bonds(atom_pairs,self.bonds_df)==True:
            try:
                vibration_dot_product=calc_vibration_dot_product_from_pairs(self.coordinates_array,self.vibration_dict,atom_pairs)
            except TypeError:
                return print('Error: no vibration array for the molecule')
